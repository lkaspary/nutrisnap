...
export async function POST(req: NextRequest) {
  try {
    ...
    const prompts: Record<string,string> = {
      label:`Read this nutrition label per serving. Always estimate if unclear.\nJSON only: {"name":"...","calories":N,"protein":N,"carbs":N,"fat":N,"serving_size":"...","confidence":"high"}`,
      text:`Food: "${text}"${clarNote}\nSearch web, then estimate. Always return a result.\nJSON only: {"name":"...","calories":N,"protein":N,"carbs":N,"fat":N,"serving_size":"...","confidence":"medium"}`,
      meal:`Analyze meal photo.${clarNote} Always estimate.\nJSON only: {"name":"...","calories":N,"carbs":N,"fat":N,"protein":N,"serving_size":"1 plate","confidence":"medium"}`,
    };
